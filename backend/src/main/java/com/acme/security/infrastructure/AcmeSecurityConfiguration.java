    ...
    public SecurityFilterChain filterChain(HttpSecurity http, AcmeJwtTokenFilter acmeJwtTokenFilter) throws Exception {
        return http
                ...
                .authorizeHttpRequests(matcherRegistry -> matcherRegistry
                        ...
                        .antMatchers(HttpMethod.POST, "/user", "/user/login", "/user/refresh", "/user/logout", "/user/logout/all").permitAll()
                        .antMatchers("/product", "/product/**").hasRole(UserRole.SELLER.getId())
                        .antMatchers("/vendingmachine", "/vendingmachine/**").hasRole(UserRole.BUYER.getId())
